  ...
  (
    ...
    useLayoutEffect(() => {
      ...
      bgRefs.current.forEach((bg, idx) => {
        if (bg) {
          if (idx === index) {
            gsap.set(bg, { opacity: 1, scale: 1, yPercent: 0 });
          } else {
            gsap.set(bg, { opacity: 0, scale: 1.04, yPercent: 0 });
          }
        }
      });
    ...
    return (
      ...
      >
        ...
        <style jsx>{`
          ...
          .fx-fixed-section { height: ${Math.max(2, total + 1)}00vh; position: relative; }
          ...
          .fx-bg:first-child .fx-bg-img {
            opacity: 1 !important;
          }
          .fx-bg:not(:first-child) .fx-bg-img {
            opacity: 0;
          }
          ...
          .fx-end { height: 10vh; display: grid; place-items: center; background: transparent; }
          .fx-fin { display: none; }
